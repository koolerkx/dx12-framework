 ...
 public:
  RenderCommandList(ID3D12GraphicsCommandList* cmd,
    ...
    DynamicUploadBuffer* object_allocator)
      : cmd_(cmd), dynamic_heap_(dynamic_heap), frame_cb_(frame_cb), object_allocator_(object_allocator),
        current_material_(nullptr), current_root_signature_(nullptr) {
  }
  ...
  void BindGlobalSRVTable(DescriptorHeapManager* manager) {
    cmd_->SetGraphicsRootDescriptorTable(
      RootSlot::ToIndex(RootSlot::DescriptorTable::GlobalSRVs),
      manager->GetGlobalSrvHeap()->GetGPUDescriptorHandleForHeapStart());
  }
  ...
  void SetFrameConstants(const FrameCB& data) {
    ...
    cmd_->SetGraphicsRootConstantBufferView(
      RootSlot::ToIndex(RootSlot::ConstantBuffer::Frame),
      frame_cb_->GetGPUAddress());
  }
  ...
  void SetObjectConstants(const ObjectCB& data) {
    ...
    cmd_->SetGraphicsRootConstantBufferView(
      RootSlot::ToIndex(RootSlot::ConstantBuffer::Object),
      allocation.gpu_ptr);
  }
  ...
  void SetMaterialData(const MaterialInstance& material_instance) {
    ...
    uint32_t material_data[4] = {
      material_instance.albedo_texture_index,
    ...
    cmd_->SetGraphicsRoot32BitConstants(
      RootSlot::ToIndex(RootSlot::Constants::MaterialData),
      4,
      material_data,
      0);
  }
