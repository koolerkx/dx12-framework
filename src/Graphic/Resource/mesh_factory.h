...
class MeshFactory {
 public:
  ...
  static bool CreateCube(ID3D12Device* device, Mesh& out_mesh, const CubeCornerColors& colors) {
    ...
    // Corner-to-vertex mapping per face (see CubeCornerColors diagram)
    ...
    for (int i = 0; i < 24; ++i) {
      const auto& c = colors[CORNER_MAP[i]];
      vertices[i].color[0] = c.x;
      vertices[i].color[1] = c.y;
      vertices[i].color[2] = c.z;
      vertices[i].color[3] = c.w;
    }
