...
UVResult CalculateFrameUV(const FrameConfig& config, uint32_t frame_index) {
  ...
  uint32_t total_frames =
    (config.orientation == Orientation::Horizontal) ? frames_per_row * frames_per_col : frames_per_col * frames_per_row;
  ...
  UVResult result;
  result.uv_offset = {(static_cast<float>(x) + 0.5f) / config.sheet_size.x, (static_cast<float>(y) + 0.5f) / config.sheet_size.y};
  result.uv_scale = {
    static_cast<float>(config.frame_size.x) / config.sheet_size.x, static_cast<float>(config.frame_size.y) / config.sheet_size.y};
